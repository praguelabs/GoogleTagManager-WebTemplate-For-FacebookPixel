...

{
  ...
  "description": "This is an unofficial Google Tag Manager template for the Facebook Pixel. Forked by Prague Labs to support Consent Mode.",
  "securityGroups": [],
  "categories": [
    "ADVERTISING",
    "ANALYTICS"
...
const consentCallback = (consentType, granted) => {
  if (consentType === 'ad_storage' && granted) {
    fbq('consent', 'grant');
  } else if (consentType === 'ad_storage') {
    fbq('consent', 'revoke');
  }
};

addConsentListener('ad_storage', consentCallback);

fbq('consent', isConsentGranted('ad_storage') ? 'grant' : 'revoke');
